...
const ArgumentEditor = ({ argIndex, operator, value, onChange, parentOperator = null }) => {
    ...
    const filteredColumns = getFilteredColumns(argIndex, operator, parentContext);
    console.log('EXPRESSION BUILDER: Filtered columns for this argument:', filteredColumns.length);

    return (
      <div style={{ padding: '10px', border: '1px solid #ddd', borderRadius: '5px', backgroundColor: '#f9f9f9' }}>
        {(requiredTypes.includes('int64') || requiredTypes.includes('float64') || requiredTypes.includes('str') || requiredTypes.includes('bool')) ? (
          <ExpressionBuilder
              expr={value}
              onChange={onChange}
