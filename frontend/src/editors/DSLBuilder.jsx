...
export default function DSLBuilder({ isOpen, onClose, onDSLComplete }) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      <div style={{ padding: '32px', minWidth: '600px', maxHeight: '80vh', overflowY: 'auto' }}>
        <div style={{
          display: 'flex',
        ...
        <div style={{
          ...
          <strong>How it works:</strong> Build a query to search through parliamentary debate contributions. 
          You can search for specific text patterns and combine multiple conditions using AND/OR logic.
        </div>
...
function DSLExpressionBuilder({ expr, onChange, parentOperator = null, argIndex = null, modalDepth = 0 }) {
  ...
  const addArgument = () => {
    if (expr.op === 'and' || expr.op === 'or') {
      const newArgs = [...expr.args, { op: 'contains', args: { pattern: '', column: 'value' } }];
      onChange({ ...expr, args: newArgs });
    }
  };
      ...
      {LEAF_OPS.includes(expr.op) && (
        <div>
          ...
          <div style={{ fontSize: '12px', color: '#666' }}>
            ...
            {expr.op === 'regex' && 'Python regex pattern - use \\b for word boundaries'}
          </div>
        </div>
      )}
      ...
      {(expr.op === 'and' || expr.op === 'or') && (
        <div>
          <div style={{
            ...
            <label style={{ fontWeight: 'bold', fontSize: '14px' }}>
              Conditions ({expr.op === 'and' ? 'ALL must be true' : 'ANY can be true'}):
            </label>
            <button
              onClick={addArgument}
              style={{
          ...
          {expr.args && expr.args.map((arg, index) => (
            <div key={index} style={{
              marginBottom: '10px',
              padding: '10px',
              border: '1px solid #ddd',
        ...
        <Modal isOpen={showModal} onClose={closeModal}>
          <div style={{ padding: '20px', minWidth: '500px' }}>
            <h3 style={{ marginBottom: '20px' }}>
              Edit {expr.op === 'not' ? 'Condition' : `Condition ${editingArgIndex + 1}`}
