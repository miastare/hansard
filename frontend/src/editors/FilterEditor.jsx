...
export default function FilterEditor({ step, onUpdate, availableInputs, tableSchemas, onBatchUpdate }) {
  ...
  return (
    <div style={{ 
      ...
      <h4 style={{ margin: '0 0 24px 0', color: '#2d3748', fontSize: '18px' }}>Filter Conditions</h4>
      ...
      <div style={{ 
        ...
        {hoveredInput && (
          <ColumnsPreview 
            columns={hoveredInput.schema.filter(col => col.dtype === 'bool')} 
            title={`${hoveredInput.inputStep.id} boolean columns`}
            isVisible={true}
          />
        )}
      </div>
